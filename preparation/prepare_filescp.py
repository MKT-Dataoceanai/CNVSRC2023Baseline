...
def prepare_ss_scp(single_speaker_rootdir, dst, split):
    ...
    random.shuffle(filelist)
    with open(f'single-speaker-{split}.scp', 'w') as fp:
        fp.write('\n'.join(filelist))
...
def prepare_ms_scp(multi_speaker_rootdir, dst, split):
    ...
    for csv in os.listdir(os.path.join(filelist_dir, 'multi-speaker')):
        ...
        for line in open(os.path.join(filelist_dir, 'multi-speaker', csv)).readlines():
            ...
            filelist.append(f"{uid}\t{vfn}\t{dst_dir}\t{landmark_dir}")
    random.shuffle(filelist)
    with open(f'multi-speaker-{split}.scp', 'w') as fp:
        fp.write('\n'.join(filelist))
...
parser.add_argument('--dataset', required=True, help='which dataset to be processed', choices=['cncvs', 'single-speaker', 'multi-speaker'])
...
if __name__== '__main__':
    ...
    elif dataset == 'single-speaker':
        prepare_ss_scp(f'{src}/single-speaker/', f'{dst}/single-speaker/', split)
    elif dataset == 'multi-speaker':
        prepare_ms_scp(f'{src}/multi-speaker/', f'{dst}/multi-speaker/', split)
